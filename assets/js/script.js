var apiKey = "5d33e1d8222d7034517ac01e13543c69";
var savedSearches = [];
...
var loadSearchHistory = function() {
    ...
    // return false if no previous saved searches
    if (!savedSearchHistory) {
        return false;
    }

    // turn search history string into array
    savedSearchHistory = JSON.parse(savedSearchHistory);

    // go through savedSearchHistory array and make entry for each item in the list
    for (var i = 0; i < savedSearchHistory.length; i++) {
        historyList(savedSearchHistory[i]);
    }
};
        ...
        .then(function(response) {
            ...
            fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${cityLat}&lon=${cityLon}&exclude=minutely,hourly,alerts&units=imperial&appid=${apiKey}`)
                ...
                .then(function(response){
                    ...
                    // add appropriate background color to uv index number
...
var fiveDayForecastSection = function(cityName) {
    // get and use data from open weather current weather api end point
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${apiKey}`)
        // get response turn into objects
        .then(function(response) {
            return response.json();
        })
        .then(function(response) {
...
$("#search-form").on("submit", function() {
    ...
    if (cityName === "" || cityName == null) {
        //send alert if input is empty
        alert("Please enter name of city.");
        event.preventDefault();
    } else {
        // if cityName is valid add to history list
...
$("#search-history-container").on("click", "p", function() {
    // get city name of entry and display weather
